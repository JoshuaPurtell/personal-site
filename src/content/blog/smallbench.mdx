...
Finally, each task is assigned a gold-standard hidden unit test suite, so developers can confidently evaluate the quality of their agent's outputs.
...
Running 100 instances of a gpt-4o-mini [ReAct baseline](https://github.com/JoshuaPurtell/SmallBench/blob/main/src/smallbench/benchmarks/bcb_a/test.py) (45% success) on BCB-Agent currently costs under \$0.50. With cheaper models, or prompt caching, diligent devs can likely get it down to under \$0.05. 
